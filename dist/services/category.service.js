...
async function categoryIsUnderAncestor(nodeId, ancestorId) {
    let current = await categoryRepository.findCategoryById(nodeId);
    while (current) {
...
export async function assertValidParent(categoryId, parentId) {
    if (parentId == null) {
        return;
    }
